...
const sessionCards = useMemo<SessionCard[]>(() => {
  ...
  const [conflictToast, setConflictToast] = React.useState<{ key: string; message: string } | null>(null);
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => setConflictToast(null), 3200);
      ...
      <div style={{ overflowY: 'auto', minHeight: 0, paddingRight: 6 }}>
        {visibleCards.map(card => {
          ...
          const handleCardClick = (): void => {
            if (conflict) {
              const message = `Conflicts with your reservation${conflict.eventTitle ? ` for ${conflict.eventTitle}` : ''}. ${conflict.whenText}`;
              setConflictToast({ key: card.key, message });
              return;
            }
            onSelect?.(event.Id, session.Id);
          };
          ...
          return (
            ...
            >
              ...
              {conflict && conflictToast?.key === card.key && (
                <div
                  style={{
                    marginTop: 10,
                    fontSize: 11,
                ...
                >
                  <div style={{ fontWeight: 600, marginBottom: 2 }}>Scheduling conflict</div>
                  <div>{conflictToast.message}</div>
                </div>
              )}
