...
export default function BookingApp({
  ...
  const isAuthRequired = Boolean(apiBaseUrl && !dataProvider);

  const [authState, setAuthState] = useState<AuthState>(() =>
    isAuthRequired ? { status: 'loading' } : { status: 'not-required' }
  );
  const [authMode, setAuthMode] = useState<'login' | 'register'>('login');
  const [authForm, setAuthForm] = useState({ email: '', password: '', displayName: '', username: '' });
  const [authPending, setAuthPending] = useState(false);
  const [authFeedback, setAuthFeedback] = useState<string | undefined>(undefined);
  ...
  const getProvider = React.useMemo(() => {
    ...
    if (apiBaseUrl && authState.status === 'authenticated') {
      return new HttpDataProvider(apiBaseUrl, authState.session.token);
    }
    return undefined;
  }, [dataProvider, apiBaseUrl, authState]);

  const usingSharePoint = !dataProvider && !apiBaseUrl;
  const authedUser = authState.status === 'authenticated' ? authState.session.user : undefined;
  const authToken = authState.status === 'authenticated' ? authState.session.token : undefined;
  const currentUserEmail = getProvider ? authedUser?.email ?? '' : context.pageContext.user.email;
  ...
  useEffect(() => {
    if (!isAuthRequired || !apiBaseUrl) {
      setAuthState((prev) => (prev.status === 'not-required' ? prev : { status: 'not-required' }));
      return;
    }
    setAuthState((prev) => (prev.status === 'authenticated' ? prev : { status: 'loading' }));
    let cancelled = false;
    const run = async (): Promise<void> => {
      ...
      try {
        const session = await authClient.me(apiBaseUrl, stored.token);
        if (!cancelled) {
        ...
        if (!cancelled) {
          storeAuth();
          setAuthState({ status: 'unauthenticated', error: err?.message || 'Please sign in' });
        }
      }
    };
    void run();
  ...
  }, [apiBaseUrl, isAuthRequired]);

  useEffect(() => {
    if (authState.status === 'unauthenticated' && authState.error) {
      setAuthFeedback(authState.error);
    }
  }, [authState]);
  ...
  const clearLoadedData = useCallback(() => {
    ...
    eventIdFromStr.clear();
    eventIdToStr.clear();
    sessionIdFromStr.clear();
    sessionIdToStr.clear();
  }, [eventIdFromStr, eventIdToStr, sessionIdFromStr, sessionIdToStr]);

  const handleAuthInputChange = useCallback(
    (field: 'email' | 'password' | 'displayName' | 'username') =>
  ...
  const handleAuthSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (authMode === 'login') {
          session = await authClient.login(apiBaseUrl, { email, password });
        } else {
          session = await authClient.register(apiBaseUrl, {
            email,
            password,
        ...
        setAuthState({ status: 'unauthenticated', error: message });
        storeAuth();
      } finally {
        setAuthPending(false);
      }
    },
    [apiBaseUrl, authForm, authMode, clearLoadedData]
  );
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      ...
      const sessionsUrl =
        ...
        `$select=Id,Title,StartDateTime,EndDateTime,Status,EventId,SessionCapacity,CapacityOverride,SlotsBooked,Details,Event/Title&$expand=Event&$orderby=StartDateTime`;
  ...
  useEffect(() => {
    ...
    if ((isAuthRequired && (!authedUser || !authToken)) || !email || !sessions.length) {
      setUserReservations([]);
      setSessionStatusOverrides({});
      return;
    }
  ...
  }, [sessions, sessionsById, eventsById, webUrl, context.spHttpClient, currentUserEmail, getProvider, sessionIdFromStr, eventIdFromStr, isAuthRequired, authToken, usingSharePoint]);
  ...
  if (isAuthRequired) {
        ...
        >
          <div
            style={{
              ...
              background: 'linear-gradient(180deg, #ffffff 0%, rgba(243, 251, 251, 0.94) 100%)',
              boxShadow: '0 18px 32px rgba(0, 57, 70, 0.16)',
              fontSize: 16,
              fontWeight: 600,
              color: '#003946',
            }}
    ...
    if (authState.status === 'unauthenticated') {
      const cardStyle: React.CSSProperties = {
        ...
        border: '1px solid rgba(0, 57, 70, 0.14)',
        background: 'linear-gradient(180deg, #ffffff 0%, rgba(243, 251, 251, 0.94) 100%)',
        boxShadow: '0 18px 32px rgba(0, 57, 70, 0.18)',
        display: 'grid',
        gap: 16,
      };
      ...
      return (
        <div
          className={styles.container + (className ? ' ' + className : '')}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '70vh', padding: 24 }}
        >
          <div style={cardStyle}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#003946' }}>
                {authMode === 'login' ? 'Sign in to book events' : 'Create your Booking App account'}
              </div>
              <div style={{ fontSize: 13, color: '#0f4c58', marginTop: 4 }}>
                {authMode === 'login'
            ...
            <button
              ...
              data-brand="ghost"
              style={{ height: 36, borderRadius: 999, fontSize: 12, fontWeight: 600 }}
              onClick={() => {
              ...
              disabled={authPending}
            >
              {authMode === 'login' ? 'Need an account? Register instead.' : 'Already have an account? Sign in.'}
            </button>
          </div>
        </div>
      );
  ...
  return (
    <div className={styles.container + (className ? ' ' + className : '')}>
      {getProvider && authedUser && (
        ...
        >
          <div style={{ fontSize: 12, color: '#0b3a42' }}>
            Signed in as <strong>{authedUser.displayName || authedUser.email}</strong>
          </div>
          <button
            type="button"
            className={styles.btnGhost}
