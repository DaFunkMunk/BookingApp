...
function addCardsIdempotent(
  ...
  const hasReview = prev.some(
    a =>
      a.type === 'review' &&
      a.payload.eventId === draft.eventId &&
      (a.payload.sessionId ?? 0) === (draft.sessionId ?? 0)
  );

  const hasConfirm = prev.some(
    a =>
      a.type === 'confirmation' &&
      a.payload.eventId === reservation.eventId &&
      (a.payload.sessionId ?? 0) === (reservation.sessionId ?? 0) &&
      a.payload.id === reservation.id
  );

  const next = [...prev];
  if (!hasReview) next.push({ type: 'review', payload: draft });
...
export default function EventDetails({
  ...
  const handleConfirmBooking = async (): Promise<void> => {
    ...
    const finishSuccess = (
      ...
      setLastReservation(reservation);
      setRecentActions(prev => addCardsIdempotent(prev, bookingDraft, reservation));
                  ...
                  <div key={i} style={{ border: '1px solid rgba(0, 57, 70, 0.12)', borderRadius: 12, padding: 14, background: 'linear-gradient(180deg, #ffffff 0%, rgba(243, 251, 251, 0.95) 100%)', boxShadow: shadows.card }}>
                    ...
                    {a.type === 'confirmation' && (
                      <>
                        ...
                        <ul style={{ marginTop: 6, paddingLeft: 16, fontSize: 12, color: '#333', lineHeight: 1.5 }}>
